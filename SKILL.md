...
- **核心 · 验收距离不减**:最近 W 轮内,**到验收的"距离"没缩小**。"距离"按判据类型取:布尔型数"还差几条未满足";连续 / 逼近型(如延迟降到 T)与维持型(如吞吐 ≥ 基线)则取**剩余差距 / 是否在退化**(基线 / 目标值从 Charter 取)。数据来自执行 Agent 每轮落进 ledger 的逐项判据状态(见交接协议)——**独立于自报归类标签,但仍是执行者写的结构化自报,不是地面真相**;能机械复验的判据应由骨架 / judge 直接复核,不信 met 标志。它专抓"自称前进、实则没碰要紧判据"的进展表演。
...
三角色治理里所有**机械判断**——轮数、预算、连续空转 K、距离窗口 W、终检触发、
三种合法退出的分派——**必须由确定性代码执行**(编排脚本 / workflow / 宿主语言的
while 与 if),agent 只出现在"跑一次尝试""判一次质量"这些步骤内部。理由是第一性的:
跳闸条件写成散文时,执行者可以在语义上讨价还价("这轮不算空转");写成
`if (stallStreak >= K) break` 时不能。监管 Agent 保留的只是代码测不了的**语义判断**
(同层打转、根假设已死);能数的数、能比的距离,一律下沉给代码。

同理,"监管凌驾执行"的实现方式优先是**结构性的**:执行者的下一轮令牌由编排层 /
监管进程发放(worker 干完即停,收不到下一条指令就动不了),而不是靠执行 agent
自觉服从一条"请停下"的消息。

可运行的参考实现见 [reference/loop-shell.workflow.js](./reference/loop-shell.workflow.js)
(Claude Code Workflow 形态;同构适配任何能在宿主循环里调用 agent 的编排层)。
它同时演示:执行自报的归类只用于**提前**跳闸(廉价信号),验收距离由代码从
逐项判据状态独立计算——自报永远不作为继续的依据。
...
- **验收按"客观性递减"降级**(核心:别让干活的自己判好不好——纯语言自评会系统性拉低质量):
  ...
  ② 验不了才上**独立 judge**:只喂【产物 + 验收规格】、不喂执行过程(信息不对称由编排层物理保证,不靠嘱咐),用**对抗式 prompt**(找哪条不达标、默认有罪)。**judge 默认换一个基座模型**——同族模型换 prompt 不构成独立,独立性来自不同的证据路径或不同的模型,不来自不同的角色名(避自偏好 bias;本 harness 用 Agent 的 model 参数或 orca 换 agent 类型);靠不住的再加"代理判据 + 抽样 / 人评";
